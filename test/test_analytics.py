...
class TestAnalytics:
    ...
    def test_load_habits(self, create_habits):
    ...
    def test_get_current_tracked_habit(self, create_habits,habit_id):
        for i,habit in enumerate(create_habits):
            currend_tracked_habit = Analytics.get_current_tracked_habit(habit_id=habit_id[i])
        assert currend_tracked_habit == habit

    def test_get_current_tracked_habit_errors(self):
        missing_id = 0
    ...
    def test_get_habits_with_frequency(self, create_habits, frequency):
        habits_with_frequency=Analytics.get_habits_with_frequency(habit_list=create_habits,frequency=frequency)
        ...
        habits_without_frequency = [habit for habit in create_habits if habit.completion.frequency!=frequency]
        assert len(habits_without_frequency) == len(create_habits) - len(habits_with_frequency)

    def test_get_habit_with_frequency_errors(self, create_habits):
        with pytest.raises(TypeError,match="Frequency not of type integer."):
            Analytics.get_habits_with_frequency(habit_list=create_habits,frequency="seven")
        with pytest.raises(TypeError,match="Object not of type Habit."):
            Analytics.get_habits_with_frequency(habit_list=[Habit(name=None,description=None),"Not a habit object."],frequency=7)

    @pytest.mark.parametrize("habit_id,result",[(1,28),(2,2),(3,14),(4,14),(5,1)])
    def test_get_longest_streak(self, habit_id,result):
        habit = Analytics.get_current_tracked_habit(habit_id=habit_id)
        assert Analytics.get_longest_streak(habit=habit) == result

    def test_get_longest_streak_errors(self):
        with pytest.raises(TypeError,match="Object not of type Habit."):
    ...
    def test_get_current_streak(self, habit_id,result):
        habit = Analytics.get_current_tracked_habit(habit_id=habit_id)
        assert Analytics.get_current_streak(habit=habit) == result

    def test_get_current_streak_errors(self):
        with pytest.raises(TypeError,match="Object not of type Habit."):
    ...
    def test_habit_to_dict_errors(self):
        with pytest.raises(TypeError,match="Object not of type Habit."):
            Analytics.habit_to_dict("No habit object.")

    def test_habit_to_dict_error(self):
        with pytest.raises(TypeError,match="Object not of type Habit."):
            Analytics.habit_to_dict("No habit object.")

    @pytest.mark.parametrize("habit_id_list",[[1,2,3,4,5]])
    def test_get_habit_ids(self, create_habits,habit_id_list):
        id_list = Analytics.get_habit_ids(habit_list=create_habits)
        for i, habit_id in enumerate(id_list):
            assert habit_id_list[i] == habit_id

    def test_get_habit_ids_error(self):
        with pytest.raises(TypeError,match="Object not of type Habit."):
            Analytics.get_habit_ids([Habit(name=None,description=None),"No habit object."])

    def test_create_new_habit(self, create_habits):
        ...
        habit1.habit_id = 1
        habit2.habit_id = 2
        assert habit1 == create_habits[0] and habit1.completion.frequency == 7
        assert habit2 == create_habits[1] and habit2.completion.frequency == 1

    def test_create_new_habit_errors(self):
    ...
    @pytest.mark.parametrize("habit_id",[1,2,3,4,5])
    def test_change_habit_name_description(self, habit_id):
        ...
        assert changed_habit == Analytics.get_current_tracked_habit(habit_id=habit_id)

    def test_change_habit_name_description_errors(self):
        missing_id = 0
        with pytest.raises(ValueError, match=f"There is no habit with id {missing_id} in the database."):
    ...
    def test_delete_habit_errors(self):
        delete_id = 7
    ...
    def test_get_marked_completed(self, habit_id, insert_date, marked_date):
    ...
    def test_get_marked_completed_error(self):
        habit_id = 7
