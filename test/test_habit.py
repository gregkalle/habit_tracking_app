...
@pytest.mark.parametrize("habit, habit_id",[(test_habit1,1),(test_habit2,2),(test_habit3,3),(test_habit4,4),(test_habit5,5)])
def test_load(habit, habit_id):
    assert habit == Habit.load(habit_id=habit_id)

@pytest.mark.parametrize("habit_list",[test_habit_list])
def test_load_all(habit_list):
